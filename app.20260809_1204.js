/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260809_1204'; // 每次部署必须更新，用于破坏浏览器缓存
...
async function verifyInvoice(buf, M, expectedFbaNo, expectedRows){
  ...
  if(M.totals && M.totals.cartons){
    ...
    for(let i=0;i<expectedRows;i++){
      const r=itemStartR+i; const v=ws.getRow(r).getCell(cc).value;
      if(v===null||v===undefined||String(v).trim()==='') throw new Error(`发票校验失败：箱数列(${cc}) 第 ${r} 行为空（应为每箱 1）`);
    }
  }
  if(M.total && M.total.col){
    ...
    for(let i=0;i<expectedRows;i++){
      ...
      const u=parseFloat(ws.getRow(r).getCell(uc).value)||0, q=parseFloat(ws.getRow(r).getCell(qc).value)||0;
      const tv=ws.getRow(r).getCell(tc).value;
      const exp=Math.round(u*q*100)/100;
      if(tv===null||tv===undefined||String(tv).trim()==='') throw new Error(`发票校验失败：总价列(${tc}) 第 ${r} 行为空（应为 ${exp}）`);
      if(Math.abs(parseFloat(tv)-exp)>0.01) throw new Error(`发票校验失败：总价列(${tc}) 第 ${r} 实测 ${tv} ≠ 单价×数量 ${exp}`);
    }
  }
  return true;
}
...
async function generateInvoice(tmpl){
  ...
  W.form.items.forEach((it,i)=>{
    ...
    if(M.item) Object.entries(M.item).forEach(([fld,col])=>{
      ...
      if(v||v===0){ const num = (fld==='qty'||fld==='declare'||fld==='boxWeight'||fld==='len'||fld==='wid'||fld==='hgt'||fld==='boxCount'||fld==='prodWeight'||fld==='cartons'); ws.getCell(effAddr(ws, col+r)).value = num?parseFloat(v):v; }
    });
    // 安速等带币种/原产地固定列
  ...
  // ===== 嵌入 SKU 商品图（来源 sku_images/，按 it.sku 取；缺图留空+告警，不阻断导出）=====
  // 索引 sku_image_index.json 在首次生成时按需 fetch 一次并缓存；每个 SKU 的图片按需 fetch 一次
  // (首次联网，之后进浏览器缓存)。多图取首张代表图。图片锚定到该模板 img 列对应物品行。
  if(M.item && M.item.img){
    try{
      if(!window.__skuImgIndex){
        try{
          const rj = await fetch(`./sku_image_index.json?v=${APP_VERSION}`);
          window.SKU_IMAGES = await rj.json();
        }catch(e){ window.SKU_IMAGES = {}; }
        window.__skuImgIndex = true;
      }
      const idx = window.SKU_IMAGES || {};
      const imgCol = colLetterToIdx(M.item.img); // 0-based
      const missImgs = [];
      for(let i=0;i<W.form.items.length;i++){
        const it = W.form.items[i];
        const sku = it.sku;
        const rel = sku ? (idx[sku] || idx[String(sku)]) : null;
        if(!rel){ if(sku) missImgs.push(sku); continue; }
        try{
          const ab = await (await fetch(`./${rel}?v=${APP_VERSION}`)).arrayBuffer();
          if(ab && ab.byteLength){
            const buf = (typeof Buffer!=='undefined') ? Buffer.from(ab) : ab;
            const imgId = wb.addImage({ buffer: buf, extension: 'jpeg' });
            ws.addImage(imgId, { tl:{ col: imgCol, row: itemStartR + i - 1 }, ext:{ width: 92, height: 92 } });
          } else missImgs.push(sku);
        }catch(e){ missImgs.push(sku); }
      }
      if(missImgs.length) console.warn(`发票有 ${missImgs.length} 个 SKU 缺商品图(已留空):`, [...new Set(missImgs)].slice(0,15));
...
async function skus(){
  ...
  <div class="card">
    ...
    <button class="btn secondary" id="reseedSk" style="margin-left:8px">↻ 重新同步主数据（从商品申报信息）</button>
    <span id="reseedMsg" style="margin-left:8px;color:var(--green)"></span>
