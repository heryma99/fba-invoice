/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260919_1630'; // 每次部署必须更新，用于破坏浏览器缓存（20260919 方案3·按票预热：FBA19PYXWFMG 全票 217 SKU 图源由 Shopify CDN 外链转存为本地 sku_images（900px/q80，共12.4MB），索引条目改指本地——该票导出全本地链路，缺图率≈0 且提速；warm_images.py 为预热工具；导出逻辑零改动）
...
function parsePackingList(text){
  ...
  // Amazon 货件装箱清单常分多段（如「原厂包装发货(N 箱)」「单件商品(N 箱)」），每段独立表头。
  // 识别所有表头行（同时含 SKU + (箱号 | 商品总数/qty)），逐段解析，避免把段标题/段表头当数据行。
  const isHeader = c => c.length >= 2
    && c.some(h => /sku/i.test(h))
    && (c.some(h => /箱号|box no|boxno|carton|ctn/i.test(h)) || c.some(h => /商品总数|总数|qty|quantity/i.test(h)));
  // 段标题（如 原厂包装发货（83 箱）/ 单件商品（1 箱））不是数据行，应跳过
  const isSectionTitle = s => /（.*箱/.test(s) || /(发货|单件|包装|混装)/.test(s);

  const headers = [];
  for (let i = 0; i < lines.length; i++) {
    const c = splitCsv(lines[i]).map(h => h.toLowerCase());
    if (isHeader(c)) headers.push(i);
  }
  if (headers.length === 0) headers.push(0);

  const out = [];
  for (let h = 0; h < headers.length; h++) {
    const hi = headers[h];
    const end = (h + 1 < headers.length) ? headers[h + 1] : lines.length;
    const hdr = splitCsv(lines[hi]).map(h => h.toLowerCase());
    const idx = {};
    for (const f in map) { const i = hdr.findIndex(h => map[f].some(k => h.includes(k))); if (i >= 0) idx[f] = i; }
    const hasBoxNoCol = ('boxNo' in idx);

    // 先扫该段元数据行（空 SKU + 「箱号」等标签），取出段级箱号，用于回填「无箱号列」的单件区
    let sectionBoxNo = '';
    for (let n = hi + 1; n < end; n++) {
      const c = splitCsv(lines[n]);
      if (c.every(x => !x)) continue;
      if ((c[0] || '').trim()) continue; // 只找空 SKU 的元数据行
      const li = c.findIndex(x => x.trim() === '箱号');
      if (li >= 0 && c[li + 1] && c[li + 1].trim()) { sectionBoxNo = c[li + 1].trim(); break; }
    }
    ...
    for (let n = hi + 1; n < end; n++) {
      ...
      if (c.every(x => !x)) continue;
      const sku = (c[0] || '').trim();
      if (!sku) continue;                 // 元数据行（箱号/箱子名称/箱重等）
      if (isSectionTitle(sku)) continue;  // 段标题（原厂包装发货(N 箱)/单件商品(N 箱)）
      ...
      // 原厂包装格式：行末「箱号」列是逗号分隔的多个真实 FBA 箱号；单件区无箱号列则用段级箱号回填
      const qtyPerBox = parseFloat(get('qtyPerBox')) || parseFloat(get('qty')) || 1;
      const boxNosStr = get('boxNo');
      let boxNos = boxNosStr ? boxNosStr.split(',').map(s=>s.trim()).filter(Boolean) : [];
      if (boxNos.length === 0 && !hasBoxNoCol && sectionBoxNo) boxNos = [sectionBoxNo];
      if (boxNos.length === 0) {
...
async function step6(box){
  ...
  $('#exportBtn').onclick = async ()=>{
    ...
    W.lastMissImgs = null; W.lastMissRowCount = 0;   // 重置缺图清单（A：导出完成消息上屏用）
    ...
    try{
      ...
      // A(20260917)：缺图清单直接上屏，用户一眼知道哪些行没图、可重导补齐
      const missNote = (Array.isArray(W.lastMissImgs) && W.lastMissImgs.length)
        ? '<div class="alert alert-warn" style="margin-top:6px">⚠️ 本票有 <b>' + W.lastMissRowCount + '</b> 行因图源下载失败未嵌图（涉及 ' + W.lastMissImgs.length + ' 个 SKU，已自动重试 ' + IMG_RETRY + ' 次）：'
          + esc(W.lastMissImgs.slice(0,30).join('、')) + (W.lastMissImgs.length>30 ? ' 等' : '')
          + '。图源均在库中，属瞬时网络抖动，重新导出即可补齐。</div>' : '';
      log.innerHTML='<div class="alert alert-ok">✅ 已导出填好的 Excel（保留原模板样式/合并/图片公式）。可在「校验·监控」看记录。</div>' + missNote;
...
const IMG_DECODE_MS = 5000;    // 单张图解码兜底超时(20260917)：onload/onerror 都不触发时 reject → 走跳过，绝不挂起
const IMG_RETRY = 2;           // 单张图失败自动重试次数(20260917 A+B)：瞬时断流/抖动重试常即成功，仍失败才计入缺图留空
const IMG_BODY_MS = 30000;     // 单张图 body(响应体)预算(20260917 两段式)：header 8s + body 30s；外链原图 1-5MB 慢速跨境 8s 不够
const IMG_INDEX_BODY_MS = 60000; // 图源索引 json body 预算(20260917)：4.7MB/39983 条跨境实测 16.5s，8s 一刀切曾致索引空→全部无图
...
// 20260917 修复嵌图卡死：fetchWithTimeout 只保护到「响应头到达」，body 读取(r.blob())无超时——
// 跨境 CDN/github.io 半路断流时 blob() 永久挂起，嵌图循环卡死(如卡在 3/230)，失败跳过逻辑无法触发。
// 20260917 二次修复（两段式超时）：body 一刀切 8s 会误杀大文件慢速下载——图源索引 json 4.7MB 跨境实测 16.5s，
//   8s 必 abort → 索引空 → 全部无图。改为：响应头 headerMs（判断连接活着）+ 收到头后 body 单独预算 bodyMs，
//   断流仍在 bodyMs 内被 abort 跳过（不挂死），慢速大文件能下完。
async function fetchBlobWithTimeout(url, headerMs, bodyMs){
  const ctrl = new AbortController();
  const abort = ()=>{ try{ ctrl.abort(); }catch(e){} };
  let id = setTimeout(abort, headerMs);            // 阶段1：响应头预算
  try{
    const r = await fetch(url, { signal: ctrl.signal, cache:'no-cache', referrerPolicy:'no-referrer' });
    if(!r.ok) throw new Error('HTTP '+r.status);
    clearTimeout(id); id = setTimeout(abort, bodyMs || headerMs);  // 阶段2：body 预算（收到头后重置）
    const blob = await r.blob();
    return blob;
  } finally{ clearTimeout(id); }
}
async function loadSkuImgIndex(){
  // 已成功加载过 → 直接返回（空结果不视为成功，允许重试，防瞬时断流把整个会话打成零图）
  if(window.__skuImgIndex && window.SKU_IMAGES && Object.keys(window.SKU_IMAGES).length) return window.SKU_IMAGES;
  const ATTEMPTS = 3;
  for(let a=1; a<=ATTEMPTS; a++){
    try{
      // ⚠️ 兼容坑(20260917 实测)：不能用 blob.json()——老 Chromium 内核(含国产浏览器/puppeteer 自带内核)没有 Blob.json，
      // 会抛 ".json is not a function" → 索引永远空 → 整票零图。用 text()+JSON.parse，全内核兼容。
      const j = JSON.parse(await (await fetchBlobWithTimeout(`./sku_image_index.json?v=${APP_VERSION}`, IMG_FETCH_MS, IMG_INDEX_BODY_MS)).text());
      if(j && Object.keys(j).length){
        window.SKU_IMAGES = j; window.__skuImgIndex = true; return j;
      }
      throw new Error('index empty');
    }catch(e){
      console.warn(`商品图索引获取失败(第${a}/${ATTEMPTS}次,跳過嵌图):`, e.message);
      if(a < ATTEMPTS) await new Promise(r=>setTimeout(r, 1500*a)); // 退避后重试：github.io 瞬时断流常见，一次失败不该零图
    }
  }
  window.__skuImgIndex = true; // 全部失败：本次会话不再反复拉大文件；嵌图自然全跳过，绝不阻断导出
...
function blobToPngBuffer(blob){
  return new Promise((resolve, reject)=>{
    ...
    const img = new Image();
    const _decTo = setTimeout(()=>{ try{ URL.revokeObjectURL(url); }catch(e){} reject(new Error('decode-timeout')); }, IMG_DECODE_MS);
    img.onload = ()=>{
      clearTimeout(_decTo);
      try{
    ...
    img.onerror = ()=>{ clearTimeout(_decTo); URL.revokeObjectURL(url); reject(new Error('decode-fail')); };
    img.src = url;
  });
}
...
async function fetchImageBuf(rel, maxSide, quality){
  ...
  if(!/^https?:\/\//i.test(rel)){
    const blob = await fetchBlobWithTimeout(`./${rel}?v=${APP_VERSION}`, IMG_FETCH_MS, IMG_BODY_MS);
  ...
  const blob = await fetchBlobWithTimeout(rel, IMG_FETCH_MS, IMG_BODY_MS);
  if(!blob || !blob.size) return null;
  try{ return { buf: await compressImageToJpeg(blob, maxSide, quality), ext: 'jpeg' }; }
  catch(e){
...
// 单图重试(20260917 A+B)：瞬时网络抖动(断流/超时)重试常即成功——失败重试 tries 次，仍失败才计入缺图留空
async function fetchImageBufRetry(rel, maxSide, quality, tries){
  let lastErr = null;
  for(let a=0; a<=tries; a++){
    try{
      const r = await fetchImageBuf(rel, maxSide, quality);
      if(r && r.buf) return r;
...
async function generateInvoice(tmpl){
  ...
  if(M.item && M.item.img && W.embedImages!==false){
    try{
      ...
      if(tasks.length){
        ...
        for(let b=0; b<tasks.length; b+=IMG_POOL){
          ...
          await Promise.all(batch.map(async tk=>{
            try{
              const res = await fetchImageBufRetry(tk.rel, tier.maxSide, tier.q, IMG_RETRY);   // 失败自动重试 IMG_RETRY 次；压缩后 ≤200KB/张
              if(res && res.buf){
      ...
      // A(20260917)：缺图清单挂到 W，供导出完成消息上屏（去重展示 SKU，行数含跨箱重复）
      W.lastMissImgs = [...new Set(missImgs)];
      W.lastMissRowCount = missImgs.length;
      if(missImgs.length) console.warn(`发票有 ${missImgs.length} 行缺商品图/获取失败(已留空，含重试):`, [...new Set(missImgs)].slice(0,15));
