/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260808_1338'; // 每次部署必须更新，用于破坏浏览器缓存
...
async function generateInvoice(tmpl){
  ...
  if(totalsRow){
    ...
    totalsRow = 0;             // 此后插入锚点走模板末尾，合计行统一在末尾重建
  }
  // 物品行：先清空已有样例行（按数据行数+缓冲清，防止样例行比实际行多）
  // 注意：img/imgUrl 是模板 DISPIMG 图片公式列，系统不写图片，跳过以免触碰共享公式越界；
  // 合计行(及其下方)跳过清空，保留其「合计」标签不被 M.item 列清空抹掉。
  const clearN = W.form.items.length + 50;
  if(M.item){
    for(let i=0;i<clearN;i++){
      const r = itemStartR + i;
      if(totalsRow && r >= totalsRow) continue; // 跳过合计行及下方
  ...
  // (itemStartR / TOTAL_RE / totalsRow 已在上方物品行清空前算好)
  const colL = n => n<1?'':n>26?String.fromCharCode(64+Math.floor((n-1)/26))+String.fromCharCode(65+(n-1)%26):String.fromCharCode(64+n);
  const insertAnchor = totalsRow || (ws.rowCount + 1); // 无合计行时插到模板末尾之后
  ...
  if(W.form.items.length > nativeAvail){
    ...
    ws.insertRow(insertAnchor, inserted); // 一次性在合计行(或末尾)前插入差额空行，合计行被自然下推
