/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260820_1839'; // 每次部署必须更新，用于破坏浏览器缓存（20260820：新增亚马逊 MUL_SKU 混箱格式解析——箱号取底部「箱号」行、子单号=同箱号、数量取「第N箱」列）
...
async function parsePackingXlsx(arrayBuffer){
  ...
  if(hasMultiSku){
    const scanBottom=(re)=>{
      for(let r=Math.min(ws.rowCount,45); r<=ws.rowCount; r++){
        const row=ws.getRow(r);
        for(let c=1;c<=row.cellCount;c++){
