/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260819_1640'; // 每次部署必须更新，用于破坏浏览器缓存（修正本地小图：本地 2409 张图全部 < 5KB 是低质量截图片段，2516 个 SKU 被正确外链图覆盖，Croc/Crystal 等肉眼可区分）
...
async function fetchImageBuf(rel){
  ...
  // 按真实 content-type 选扩展名，webp/gif 用 canvas 重编码为 PNG（ExcelJS 不支持 webp/gif）
  const r = await fetchWithTimeout(rel, IMG_FETCH_MS);
  const ab = await r.arrayBuffer();
  if(!ab || !ab.byteLength) return null;
  const ct = (r.headers && r.headers.get) ? (r.headers.get('content-type') || '') : '';
  let buf = (typeof Buffer!=='undefined') ? Buffer.from(ab) : ab;
  let ext = 'jpeg';
  if(ct.indexOf('png') >= 0) ext = 'png';
  else if(ct.indexOf('webp') >= 0 || ct.indexOf('gif') >= 0){
    try{ buf = await blobToPngBuffer(new Blob([ab])); ext = 'png'; }catch(e){ /* 解码失败：保留原始字节让上层按 jpeg 试，失败则跳过 */ }
  }
  return { buf, ext };
}
      ...
      if(tasks.length){
        for(let b=0; b<tasks.length; b+=IMG_POOL){
          ...
          await Promise.all(batch.map(async tk=>{
            try{
              const res = await fetchImageBuf(tk.rel);   // 5s 超时；本地返回 jpeg，外链按真实类型(可能 png 或 webp→png)
              if(res && res.buf){
