/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260917_2016'; // 每次部署必须更新，用于破坏浏览器缓存（20260917：修复嵌图导出卡死——fetchWithTimeout 超时只覆盖到响应头，body 读取无超时致 CDN 断流时 blob() 永久挂起（卡在 3/230）；新增 fetchBlobWithTimeout 将 fetch+body 纳入同一超时控制，压缩解码加 5s 兜底；失败图走既有跳过逻辑，绝不阻断导出）
...
// 本函数把 fetch+body 读取放进同一个超时控制器：定时器到 body 读完才清，body 断流同样被 abort → 走既有失败跳过。
...
async function loadSkuImgIndex(){
  ...
  try{
    window.SKU_IMAGES = await (await fetchBlobWithTimeout(`./sku_image_index.json?v=${APP_VERSION}`, IMG_FETCH_MS)).json();
...
async function fetchImageBuf(rel, maxSide, quality){
  ...
  if(!/^https?:\/\//i.test(rel)){
    const blob = await fetchBlobWithTimeout(`./${rel}?v=${APP_VERSION}`, IMG_FETCH_MS);
  ...
  const blob = await fetchBlobWithTimeout(rel, IMG_FETCH_MS);
  if(!blob || !blob.size) return null;
  try{ return { buf: await compressImageToJpeg(blob, maxSide, quality), ext: 'jpeg' }; }
  catch(e){
