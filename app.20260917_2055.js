/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260917_2055'; // 每次部署必须更新，用于破坏浏览器缓存（20260917 三次修复：图源索引加载加 3 次退避重试——github.io 瞬时断流一次失败即缓存空索引导致整票零图；两段式超时(header 8s+body 30s/60s)保留，断流跳过能力保留）
...
async function loadSkuImgIndex(){
  ...
  for(let a=1; a<=ATTEMPTS; a++){
    try{
      const j = await (await fetchBlobWithTimeout(`./sku_image_index.json?v=${APP_VERSION}`, IMG_FETCH_MS, IMG_INDEX_BODY_MS)).json();
