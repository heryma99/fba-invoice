/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260809_1014'; // 每次部署必须更新，用于破坏浏览器缓存
...
async function verifyInvoice(buf, M, expectedFbaNo, expectedRows){
  ...
  // 3) 物品行数核对（boxNo/boxLabel 列非空行数 == 预期箱数），仅扫物品行范围(O(N))
  const boxCol = M.item && (M.item.boxNo||M.item.boxLabel);
  if(boxCol){
    const boxColNum = ws.getColumn(boxCol.replace(/[0-9]/g,'')).number; // 列字母->列号
    let n=0;
    for(let i=0;i<expectedRows;i++){ const r=itemStartR+i; const c=ws.getRow(r).getCell(boxColNum).value; if(c!==null && c!==undefined && String(c).trim()!=='') n++; }
