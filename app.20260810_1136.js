/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260810_1136'; // 每次部署必须更新，用于破坏浏览器缓存
...
async function seedIfEmpty(){
  ...
  const SKUS_SEED_VER = 8; // bump: 商品申报信息(8/3源)全量重同步为SKU主数据(主键=商品编码B,13019条),强制重灌IndexedDB
...
const IMG_EMBED_CAP = 300;     // 单票最多嵌图数，防万箱货把 ExcelJS 写爆
const IMG_FETCH_MS = 5000;     // 单张图/索引的联网超时(ms)
const IMG_POOL = 8;            // 并发拉取上限
async function fetchWithTimeout(url, ms){
  const ctrl = new AbortController();
  const id = setTimeout(()=>{ try{ ctrl.abort(); }catch(e){} }, ms);
  try{ const r = await fetch(url, { signal: ctrl.signal, cache:'no-cache' }); if(!r.ok) throw new Error('HTTP '+r.status); return r; }
...
async function fetchImageBuf(rel){
      ...
      if(tasks.length){
        for(let b=0; b<tasks.length; b+=IMG_POOL){
          ...
          await Promise.all(batch.map(async tk=>{
            try{
              const buf = await fetchImageBuf(tk.rel);   // 5s 超时
              if(buf){
                const imgId = wb.addImage({ buffer: buf, extension: 'jpeg' });
                ws.addImage(imgId, { tl:{ col: imgCol, row: itemStartR + tk.i - 1 }, ext:{ width: 92, height: 92 } });
