/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260917_2040'; // 每次部署必须更新，用于破坏浏览器缓存（20260917 二次修复：嵌图超时改两段式——header 8s + body 单独预算(图 30s/索引 60s)。此前 body 一刀切 8s 误杀 4.7MB 图源索引慢速下载(跨境实测 16.5s)→索引空→导出零图片；断流跳过能力保留）
...
async function loadSkuImgIndex(){
  ...
  try{
    window.SKU_IMAGES = await (await fetchBlobWithTimeout(`./sku_image_index.json?v=${APP_VERSION}`, IMG_FETCH_MS, IMG_INDEX_BODY_MS)).json();
