/* ============================================================
   ...
   ============================================================ */
'use strict';
const APP_VERSION = '20260729_1054'; // 每次部署必须更新，用于破坏浏览器缓存
...
async function seedDefaultTemplates(force=false){
  ...
  for(const [key, file] of Object.entries(TPL_FILES)){
    const id = 'tmpl_'+key;
    if(!force && have.has(id)) continue; // 已有则跳过,不重复
    try{
...
function step1(box){
  ...
  <div class="card-row" style="display:flex;gap:16px;flex-wrap:wrap">
    ...
    <div class="card" style="flex:1;min-width:340px">
      ...
      <div class="hint">上传本机装箱清单（Excel/CSV），系统解析物品行。<b>物流渠道需手动选择</b>（文件中不含这些信息）。若能从文件表头识别出 FBA 号，物流商可自动反查。</div>
...
/* 卡片 B 未命中交接清单 → 全手动选 */
function renderCardBManual(el){
  el.innerHTML = `
    <div class="card" style="margin-top:10px;border-color:#c53030">
      <div class="hint warn">⚠️ 未识别到 FBA 货件号（或不在交接清单中），请手动选择物流商和渠道</div>
...
function parsePackingList(text){
  ...
  for(let n=1;n<lines.length;n++){
    ...
    out.push({boxLabel:get('boxLabel'),boxNo:get('boxNo'),sku:get('sku'),nameCn:get('nameCn'),nameEn:get('nameEn'),qty:get('qty')||1,declare:get('declare'),material:get('material'),purpose:get('purpose'),hs:get('hs'),brand:get('brand'),model:get('model'),boxWeight:get('boxWeight'),prodWeight:'',len:get('len'),wid:get('wid'),hgt:get('hgt'),elec:(get('elec')||'N').toUpperCase().startsWith('Y')?'Y':'N',magnet:(get('magnet')||'N').toUpperCase().startsWith('Y')?'Y':'N',saleUrl:get('saleUrl'),cost:''});
  }
  return out;
}
...
function step4(box){
  ...
  <div class="card">
    ...
    <label>模板（物流商）</label>
    <select id="selTmpl">${tmpls.length? tmpls.map(t=>`<option value="${t.id}" ${t.id===W.selTmpl?'selected':''}>${esc(t.物流商)} (v${t.版本||1}, ${t.状态})</option>`).join('') : '<option>（无可用模板）</option>'}</select>
    ${tmpls.length?'':`
